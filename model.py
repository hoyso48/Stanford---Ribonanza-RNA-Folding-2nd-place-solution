...
import math
from modules import Conv1DBlock, MaskedConv2d, AltBlock, get_alibi
...
class RNARegModel(nn.Module):
    ...
    def forward(self, data):
        ...
        result = {}
        result['output'] = react_pred

        mae, react_sample_size = mae_loss(react_pred, y, mask, weights=torch.log1p(data['snr'].clip(0)).clip(0,10).unsqueeze(1))
        result['losses'] = {}
        result['metrics'] = {} 
        result['sample_size'] = mask.long().sum().detach()
        result['react_sample_size'] = react_sample_size.detach()
        
        result['losses']['mae_loss'] = mae
        result['metrics']['mae'] = mae_loss(react_pred, y, mask, weights=None)[0].detach()
            
        return result

def get_model(cfg):
    return RNARegModel(dim=cfg.dim, num_layers=cfg.num_layers, num_heads=cfg.num_heads)